...
const CandidateDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'dashboard' || activeTab === 'quizzes') {
      ...
      }, 5000); // UI updates naturally due to currentTime state every second anyway, but we poll for real backend changes too
    }
  ...
  const liveUnattemptedQuizzes = quizzes.filter(q => {
    const isLive = q.isLive && new Date(q.startTime) <= currentTime && new Date(q.endTime) >= currentTime;
    return isLive && !completedAttempts.some(a => a.quiz?._id === q._id);
  });
            ...
            <div className="animate-fade-in space-y-6">
              ...
              {quizzes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {quizzes.filter(q => q.isLive).map(quiz => {
                    const startTime = quiz.startTime ? new Date(quiz.startTime) : null;
                    const endTime = quiz.endTime ? new Date(quiz.endTime) : null;
                    
                    const isLive = quiz.isLive && startTime <= currentTime && endTime >= currentTime;
                    const isCompleted = quiz.isCompleted || completedAttempts.some(a => a.quiz?._id === quiz._id);
                    const isNotStarted = startTime && currentTime < startTime;
                    const isUpcoming = isNotStarted && (startTime - currentTime) > 10 * 60 * 1000;
                    const isEnvReady = isNotStarted && (startTime - currentTime) <= 10 * 60 * 1000;
                      ...
                      <div key={quiz._id} className="bg-white/70 backdrop-blur-xl border border-white/40 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-2xl hover:-translate-y-2 transition-all flex flex-col group relative overflow-hidden">
                        ...
                        {isCompleted ? (
                          ...
                          <button disabled className="w-full py-3 rounded-xl font-bold flex items-center justify-center bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200 shadow-sm">
                            Not Started <Play size={16} className="ml-2 opacity-50"/>
                          </button>
                        ) : (
